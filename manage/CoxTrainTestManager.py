...
class CoxTrainTestManager:
    ...
    def leave_one_out_cv(
            self,
            X: np.ndarray,
            y: np.ndarray[tuple[int, float]]) -> tuple[
                                                np.ndarray,
                                                np.ndarray]:
        """
        ...
        for i, (train_index, test_index) in enumerate(folds):
            ...
            self.model.train(X[train_index],
                             y[train_index])
            ...
            risk_scores[test_index] = self.model.predict_risk_score(
                X[test_index])
            risk_classes[test_index] = self.model.predict_class(
                risk_scores[test_index], cutoff)
