...
class ClassificationMetrics:
    ...
    def compute_roc_curve(y_true: np.ndarray,
                          y_score: np.ndarray) -> tuple[np.ndarray,
                                                        np.ndarray,
                                                        np.ndarray]:
        """
        ...
        ### Parameters :
        - y_true (n_samples, ) : numpy array containing the correct classes for
        each sample
        - y_score (n_samples, ): numpy array containing the output score
        predicted by the model (class 1 probability)
    ...
    @staticmethod
    def compute_sensitivity_specificity(y_true: np.ndarray,
                                        y_pred: np.ndarray) -> tuple[float,
                                                                     float]:
        """
        Compute the sensitivity and the specificity of the prediction.

        ### Parameters :
        - y_true (n_samples, ) : numpy array containing the correct class for
        each sample
        - y_pred (n_samples, ): numpy array containing the predicted class for
        each sample
