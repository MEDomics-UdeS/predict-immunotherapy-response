...
class SurvivalMetrics:
    ...
    @staticmethod
    def get_c_index(status: np.ndarray,
                    time: np.ndarray,
        ...
        ### Parameters :
        - status (n_samples, ) : the event status for each sample (1 if event
        ...
        """
        return np.round(
            concordance_index_censored(status, time, risk_scores)[0], 2)

    @staticmethod
    def estimate_survival_curve(status: np.ndarray,
                                time: np.ndarray) -> tuple[np.ndarray,
                                                           np.ndarray]:
        """
    ...
    @staticmethod
    def get_p_value_log_rank_test(status: np.ndarray,
                                  time: np.ndarray,
                                  risk_classes: np.ndarray) -> float:
        """
        ...
        y = np.array(list(zip(status, time)),
                     dtype=[('status', '?'), ('time surviving', '<f8')])
